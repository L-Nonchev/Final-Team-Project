<?php
function updateProfilBanner ($fileOnServer, $fileRealName){
	//get email form user
	...
	$path = '../assets/images/user-banners/'.$fileRealName;
	...
	if ($mime === $format){
		...
		if (is_uploaded_file($fileOnServer)){
			...
			if (move_uploaded_file($fileOnServer, $path)){
				...
				smart_resize_image($file , null, 1583 , 350 , false , $resizedFile , false , false ,100 );

				$result = $userData->updateBanner($user->userId, $fileRealName);
